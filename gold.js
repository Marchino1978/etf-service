...
async function fetchWithRetry(url, retries = 3, delay = 1000) {
  for (let i = 0; i < retries; i++) {
    ...
    const mid = $('span[source="lightstreamer"][table="quotes"][item="979663@1"][field="mid"]').text().trim();
    if (mid) {
      return { $, mid };
    }
...
export default async function getGOLD() {
  ...
  const bid = $('span[source="lightstreamer"][table="quotes"][item="979663@1"][field="bid"]').text().trim();
  const ask = $('span[source="lightstreamer"][table="quotes"][item="979663@1"][field="ask"]').text().trim();
  const change = $('span[source="lightstreamer"][table="quotes"][item="979663@1"][field="change"]').text().trim();

  const prevClose = getPreviousClose("GOLD");
  let dailyChange = "";
  ...
  if (mid) saveClose("GOLD", mid);

  return {
    source: "LS-TC",              // 🔴 AGGIUNTO
    symbol: "GOLD",               // 🔴 AGGIUNTO
