...
~~~
{
  "name": "etf-service",
  "version": "1.0.0",
  "description": "Servizio per scraping ETF da Lang & Schwarz",
  "type": "module",
  "main": "core/server.js",
  "scripts": {
...
# Esclude: txt/, .git/, node_modules
# Per i file HTML: usa tilde fences ~~~ e indentazione a 4 spazi

mkdir -p txt

wrap_with_tilde_fence() {
  # Stampa contenuto avvolto da ~~~ e con indentazione a 4 spazi per ogni riga
  echo "~~~"
...
dump_folder() {
  ...
  for file in "$folder"/*; do
    ...
    case "$file" in
      *.html)
        wrap_with_tilde_fence < "$file" >> "$output"
        ;;
      *)
        echo "~~~" >> "$output"
        cat "$file" >> "$output"
...
# Dump di ogni sottocartella, esclusioni
for dir in */; do
  [ -d "$dir" ] || continue
  foldername=$(basename "$dir")
  case "$foldername" in
    txt|.git|node_modules) continue ;;
